// lcdvs.c - funcoes para lcd HD44780 a 4 bits - procesadores VLSI VS1005 - vide arquivo lcdvs.h
...
static u_int16 port;      //signals plus data - upper 4 bits hardwired to nibble bits 4-7
...
void lcd_send_nibble(u_int16 display, u_int16 n, u_int16 rs){
      ...
      DelayMicroSec(1);   //450 ns no minimo
      msk=0x0F;  if(rs)msk = 0x0E;
      GPIO_WritePort(CLEAR_MASK, msk);  // enable=0
      DelayMicroSec(1);   //450 ns no minimo
      GPIO_WritePort(CLEAR_MASK, 0x0F);  // finally works!
}

void lcd_putc(u_int16 display, u_int16 datachar){ //done
     GPIO_WritePort(port, 0x01);  //rs=1 en=0
     DelayMicroSec(1);    // 140 ns minimo
     lcd_send_nibble(display, datachar, 0x01);
     lcd_send_nibble(display, (datachar << 4), 0x01);
     DelayMicroSec(50);   //40 us minimo ate o final da funcao
}

void lcd_send_command(u_int16 display, u_int16 comm){ //done
     GPIO_WritePort(port, 0);  //rs=0 en=0
     DelayMicroSec(1);    // 140 ns minimo
     lcd_send_nibble(display, comm, 0);
     lcd_send_nibble(display, (comm << 4), 0);
     DelayMicroSec(240);  // valor minimo 200 us
}
...
// lcd_cl3anline - cleans a line  with num_pos digits:
...
void lcdmsgl(u_int16 display, char *string, u_int16 len, u_int16 y){  // prints messages in several lines
     u_int16 leng, printed;
     printed=0;  leng = strlen(string);
     lcd_cleanline(display, y);  lcd_cleanline(display, y+1);
...
void textscrollb(u_int16 display, char *text, u_int16 line, u_int16 nmbrchrs, u_int16 *scposi){   // textscrollb - faz um texto longo ser movimentado em uma linha do display lcd
     u_int16 i, pos, aux, l;
     l = strlen(text);
     pos=*scposi; i=0;
     ...
     if(text[*scposi]==0)*scposi=0;
     else if(l>nmbrchrs) (*scposi)++;
}

void textscroll(u_int16 display, char *text, u_int16 line, u_int16 nmbrchrs, u_int16 *scposi){   // textscroll - faz um texto longo ser movimentado em uma linha do display lcd
